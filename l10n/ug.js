OC.L10N.register(
    "text2image_helper",
    {
    "Image generation ready" : "رەسىم ھاسىل قىلىش تەييار",
    "View" : "كۆرۈش",
    "Delete" : "ئۆچۈر",
    "AI image generation" : "AI رەسىم ھاسىل قىلىش",
    "No text to image processing provider available" : "رەسىم بىر تەرەپ قىلغۇچىغا تېكىست يوق",
    "Image generation has been deleted." : "رەسىم ھاسىل قىلىش ئۆچۈرۈلدى.",
    "Image generation not found." : "رەسىم ھاسىل قىلىنمىدى.",
    "Image generation failed." : "رەسىم ھاسىل قىلىش مەغلۇب بولدى.",
    "Image file names could not be fetched from database" : "رەسىم ھۆججىتىنىڭ ئىسمىنى سانداندىن تاپقىلى بولمايدۇ",
    "Image request error" : "رەسىم تەلەپ قىلىش خاتالىقى",
    "Image file not found in database" : "سانداندا رەسىم ھۆججىتى تېپىلمىدى",
    "Image file not found" : "رەسىم ھۆججىتى تېپىلمىدى",
    "Text2Image helper" : "Text2 تەسۋىر ياردەمچىسى",
    "A Smart Picker for interfacing with text-to-image providers" : "تېكىستتىن رەسىم تەمىنلىگۈچىلەر بىلەن ئارىلىشىشتىكى ئەقلىي ئىقتىدارلىق تاللىغۇچ",
    "⚠️ This app is now deprecated. The feature it provides is now included in the [Assistant app](https://apps.nextcloud.com/apps/assistant).\n\nThis app adds a Smart Picker for interfacing with text-to-image providers like the on-premises stable diffusion integration.\n\nThe Smart Picker enables you to generate an image or multiple images based on a text prompt.\n\nIf multiple images are requested and/or the generation will take a long time. The user can request to be notified\nonce the generated image(s) are ready." : "App بۇ ئەپنىڭ ۋاقتى ئۆتكەن. ئۇ تەمىنلىگەن ئىقتىدار ھازىر [ياردەمچى ئەپ] (https://apps.nextcloud.com/apps/assistant) غا كىرگۈزۈلدى.\n\nبۇ ئەپ نەق مەيداندىكى مۇقىم تارقىلىشچان بىرىكتۈرۈشكە ئوخشاش تېكىستتىن رەسىم تەمىنلىگۈچىلەر بىلەن ئۆز-ئارا تەسىر كۆرسىتىش ئۈچۈن Smart Picker قوشىدۇ.\n\nSmart Picker تېكىست ئەسكەرتمىسى ئاساسىدا رەسىم ياكى كۆپ رەسىم ھاسىل قىلالايسىز.\n\nئەگەر بىر قانچە رەسىم تەلەپ قىلىنسا ۋە ياكى ئەۋلادلارغا ئۇزۇن ۋاقىت كېتىدۇ. ئىشلەتكۈچىگە ئۇقتۇرۇش قىلىشنى تەلەپ قىلسا بولىدۇ\nھاسىل قىلىنغان رەسىم (لار) تەييارلانغاندىن كېيىن.",
    "Text2Image Helper admin options saved" : "Text2Image ياردەمچى باشقۇرغۇچى تاللانمىلىرى ساقلاندى",
    "Failed to save Text2Image Helper admin options" : "Text2Image ياردەمچى باشقۇرغۇچى تاللانمىلىرىنى ساقلىيالمىدى",
    "Image generation - Text2Image Helper" : "رەسىم ھاسىل قىلىش - Text2 تەسۋىر ياردەمچىسى",
    "Here you can define the settings for the Text2Image helper smart picker." : "بۇ يەردە Text2Image ياردەمچىسى ئەقلىي ئىقتىدارلىق تاللىغۇچنىڭ تەڭشىكىنى بەلگىلىيەلەيسىز.",
    "Max generation idle time (days): " : "ئەڭ چوڭ ئەۋلاد بىكار ۋاقىت (كۈن): ",
    "Maximum number of days an image can be idle (not viewed) before it is deleted" : "رەسىم ئۆچۈرۈلۈشتىن بۇرۇن ئەڭ كۆپ كۈن بىكار (كۆرگىلى بولمايدۇ)",
    "Image generation failed" : "رەسىم ھاسىل قىلىش مەغلۇب بولدى",
    "This generation has no visible images" : "بۇ بىر ئەۋلاد كىشىلەرنىڭ كۆرۈنمەيدىغان رەسىملىرى يوق",
    "Unexpected server response" : "كۈتۈلمىگەن مۇلازىمېتىر ئىنكاسى",
    "Rate limit reached. Please try again later." : "باھا چېكىگە يەتتى. كېيىن قايتا سىناڭ.",
    "Unknown server query error" : "نامەلۇم مۇلازىمېتىر سوئال خاتالىقى",
    "Image generation" : "رەسىم ھاسىل قىلىش",
    "Edit visible images" : "كۆرۈنگەن رەسىملەرنى تەھرىرلەڭ",
    "Click to toggle generation visibility" : "ئەۋلاد كۆرۈنۈشچانلىقىنى ئۆزگەرتىش ئۈچۈن چېكىڭ",
    "Generated image" : "ھاسىل قىلىنغان رەسىم",
    "Estimated generation time left: " : "مۆلچەرلەنگەن ئەۋلاد ۋاقتى: ",
    "The generated image is shown once ready." : "ھاسىل قىلىنغان رەسىم تەييارلانغاندىن كېيىن كۆرسىتىلىدۇ.",
    "A hedgehog laid back on a couch" : "بىر كىرپىك ئورۇندۇققا ياتقۇزۇلدى",
    "Regenerate" : "قايتا ھاسىل",
    "Preview" : "ئالدىن كۆرۈش",
    "Image generation cancel error" : "رەسىم ھاسىل قىلىش خاتالىق",
    "Unknown image generation cancel error" : "نامەلۇم رەسىم ھاسىل قىلىش خاتالىقنى ئەمەلدىن قالدۇرىدۇ",
    "Unexpected response from server." : "مۇلازىمېتىردىن كۈتۈلمىگەن جاۋاب.",
    "Image generation error" : "رەسىم ھاسىل قىلىش خاتالىقى",
    "Unknown image generation error" : "نامەلۇم رەسىم ھاسىل قىلىش خاتالىقى",
    "You will be notified when the image generation is ready." : "رەسىم ھاسىل قىلىش تەييار بولغاندا سىزگە ئۇقتۇرۇلىدۇ.",
    "Notify when ready error" : "تەييار خاتالىق بولغاندا ئۇقتۇرۇش قىلىڭ",
    "Unknown notify when ready error" : "تەييار خاتالىق كۆرۈلگەندە نامەلۇم",
    "ERROR: No image generation provider available" : "خاتالىق: رەسىم ھاسىل قىلغۇچى تەمىنلىگۈچى يوق",
    "AI Image Generation" : "AI Image Generation",
    "Show/hide advanced options" : "ئىلغار تاللاشلارنى كۆرسىتىش / يوشۇرۇش",
    "Advanced options" : "ئىلغار تاللاشلار",
    "Preview image generation by AI" : "سۈنئىي ئەقىل ئارقىلىق رەسىم ھاسىل قىلىشنى ئالدىن كۆرۈڭ",
    "Notify when ready" : "تەييار بولغاندا خەۋەر قىلىڭ",
    "Submit image(s) generated by AI" : "سۈنئىي ئەقىل ھاسىل قىلغان رەسىم (لەر) نى يوللاڭ",
    "Send" : "يوللا",
    "Include the prompt in the result" : "نەتىجىنى تېزلىكتە ئۆز ئىچىگە ئالىدۇ",
    "Number of results" : "نەتىجە سانى",
    "Link copied to clipboard" : "ئۇلىنىش چاپلاش تاختىسىغا كۆچۈرۈلدى",
    "Failed to copy link to clipboard" : "چاپلاش تاختىسىغا ئۇلىنىش كۆچۈرۈلمىدى",
    "Copy link to clipboard" : "ئۇلىنىشنى چاپلاش تاختىسىغا كۆچۈرۈڭ"
},
"nplurals=2; plural=(n != 1);");
