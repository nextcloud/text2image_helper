OC.L10N.register(
    "text2image_helper",
    {
    "Image generation ready" : "جاهز لتوليد الصِّوَر",
    "View" : "عرض",
    "Delete" : "حذف",
    "AI image generation" : "توليد صور بالذكاء الصناعي",
    "No text to image processing provider available" : "لا يوجد مزود لتحويل النص إلى صورة",
    "Image generation has been deleted." : "تم حذف توليد الصِّوَر.",
    "Image generation not found." : "تعذّر العثور على توليد الصِّوَر.",
    "Image generation failed." : "فشل في توليد الصور.",
    "Image file names could not be fetched from database" : "تعذّر جلب أسماء ملفات الصِّوَر من قاعدة البيانات",
    "Image request error" : "خطأ في طلب الصورة",
    "Image file not found in database" : "ملف الصورة غير موجود في قاعدة البيانات",
    "Image file not found" : "ملف الصورة غير موجود",
    "Text2Image helper" : "مُساعِد التحويل من نص إلى صورة Text2Image",
    "A Smart Picker for interfacing with text-to-image providers" : "لاقط ذكي للتواصل مع مزودي التحويل من نص إلى صورة",
    "⚠️ This app is now deprecated. The feature it provides is now included in the [Assistant app](https://apps.nextcloud.com/apps/assistant).\n\nThis app adds a Smart Picker for interfacing with text-to-image providers like the on-premises stable diffusion integration.\n\nThe Smart Picker enables you to generate an image or multiple images based on a text prompt.\n\nIf multiple images are requested and/or the generation will take a long time. The user can request to be notified\nonce the generated image(s) are ready." : "⚠️ هذا التطبيق مُهمل الآن. الميزة التي كان يوفرها هذا التطبيق مضمنة الآن في [تطبيق المساعد Assiatant App] (https://apps.nextcloud.com/apps/assistant). \n\nيضيف هذا التطبيق لاقطاً ذكيًا للتواصل مع مزوديي خدمة تحويل النص إلى صورة مثل تكامل الانتشار المستقر المحلي on-premises stable diffusion integration. \n\nيُمكّنك اللاقط الذكي من توليد صورة أو صور متعددة بناءً على مطالبة نصية. \n\nإذا تم طلب صور متعددة سيستغرق الطلب و/أو التوليد وقتًا طويلاً. يمكن للمستخدم أن يطلب أن يتم إعلامه بمجرد أن تصبح الصورة (الصور) االمولدة جاهزة.",
    "Image generation - Text2Image Helper" : "توليد الصِّوَر - مُساعِد التحويل من نص إلى صورة Text2Image",
    "Here you can define the settings for the Text2Image helper smart picker." : "هُنا يمكنك تعريف إعدادات اللاقط الذكي لمُساعد تحويل النص إلى صورة Text2Image.",
    "Max generation idle time (days): " : "الحد الأقصى (بالأيام) لفترة الخمول في التوليد generation idle:",
    "Maximum number of days an image can be idle (not viewed) before it is deleted" : "أقصى عدد من الأيام يمكن أن تبقى الصورة في وضع الخمول (بدون عرض) قبل حذفها",
    "Text2Image Helper admin options saved" : "تمّ حفظ خيارات المدير لمساعد التحويل من نص إلى صورة Text2Image Helper",
    "Failed to save Text2Image Helper admin options" : "تعذّر حفظ خيارات المدير لمساعد التحويل من نص إلى صورة Text2Image Helper",
    "Image generation" : "توليد الصورة",
    "Edit visible images" : "تعديل صور مرئية",
    "Click to toggle generation visibility" : "إضغَط لتبديل وضعية رؤية التوليد",
    "Generated image" : "صورة مُولّدة",
    "This generation has no visible images" : "التوليد ليس فيه أي صور مرئية",
    "Estimated generation time left: " : "المتبقى من الزمن المقدر للتوليد:",
    "The generated image is shown once ready." : "سيتم عرض الصور المولدة بمجرد تجهيزها",
    "Image generation failed" : "فشل في توليد الصورة",
    "Unexpected server response" : "استجابة غير متوقعة من الخادم",
    "Rate limit reached. Please try again later." : "تمّ استنفاذ حصتك في الخدمة. يرجى المحاولة في وقت آخر.",
    "Unknown server query error" : "خطأ غير معروف عند استعلام الخادم",
    "ERROR: No image generation provider available" : "خطأ: لا يوجد أي مزود لتوليد الصور",
    "AI Image Generation" : "توليد الصور بالذكاء الاصطناعي",
    "Preview" : "مُعاينة",
    "Show/hide advanced options" : "إظهار/إخفاء الخيارات المتقدمة",
    "Advanced options" : "الخيارات المتقدمة",
    "Preview image generation by AI" : "معاينة الصور المولدة بالذكاء الاصطناعي",
    "Notify when ready" : "أعلِمني عندما تكون جاهزاً",
    "Submit image(s) generated by AI" : "إرسال الصورة ( أو الصور) المولدة بالذكاء الاصطناعي",
    "Send" : "أرسِل",
    "Include the prompt in the result" : "قم بتضمين المَحَثّ prompt في النتيجة",
    "Number of results" : "عدد النتائج",
    "A hedgehog laid back on a couch" : "القُنفُد مُستلقٍ على الأريكة",
    "Regenerate" : "إعادة التوليد",
    "Image generation cancel error" : "حدث خطأ عند إلغاء توليد الصور",
    "Unknown image generation cancel error" : "حدث خطأ غير معروف عند إلغاء توليد الصور",
    "Unexpected response from server." : "استجابة غير متوقعة من الخادم.",
    "Image generation error" : "حدث خطأ عند توليد الصور",
    "Unknown image generation error" : "حدث خطأ غير محدد عند توليد الصور",
    "You will be notified when the image generation is ready." : "سوف يتم إعلامك عندما يجهَز توليد الصور.",
    "Notify when ready error" : "حدث خطأ عند الإعلام بالجاهزية",
    "Unknown notify when ready error" : "حدث خطأ غير محدد عند الإعلام بالجاهزية",
    "Copy link to clipboard" : "انسخ الرابط إلى الحافظة",
    "Link copied to clipboard" : "تمّ نسخ الرابط إلى الحافظة",
    "Failed to copy link to clipboard" : "تعذّر نسخ الرابط إلى الحافظة"
},
"nplurals=6; plural=n==0 ? 0 : n==1 ? 1 : n==2 ? 2 : n%100>=3 && n%100<=10 ? 3 : n%100>=11 && n%100<=99 ? 4 : 5;");
