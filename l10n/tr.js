OC.L10N.register(
    "text2image_helper",
    {
    "Image generation ready" : "Görsel oluşturma hazır",
    "View" : "Görüntüle",
    "Delete" : "Sil",
    "AI image generation" : "Yapay zeka ile görsel oluşturma",
    "No text to image processing provider available" : "Kullanılabilecek bir yazıdan görsel oluşturma işleyici hizmet sağlayıcısı yok",
    "Image generation has been deleted." : "Görsel oluşturma silindi.",
    "Image generation not found." : "Görsel oluşturma bulunamadı.",
    "Image generation failed." : "Görsel oluşturulamadı.",
    "Image file names could not be fetched from database" : "Görsel dosyası adları veri tabanından alınamadı",
    "Image request error" : "Görsel isteğinde sorun çıktı",
    "Image file not found in database" : "Görsel dosyası veri tabanında bulunamadı",
    "Image file not found" : "Görsel dosyası bulunamadı",
    "Text2Image helper" : "Text2Image yardımcısı",
    "A Smart Picker for interfacing with text-to-image providers" : "Arayüz yazıdan görüntü oluşturma hizmeti sağlayıcıları için akıllı seçici",
    "⚠️ This app is now deprecated. The feature it provides is now included in the [Assistant app](https://apps.nextcloud.com/apps/assistant).\n\nThis app adds a Smart Picker for interfacing with text-to-image providers like the on-premises stable diffusion integration.\n\nThe Smart Picker enables you to generate an image or multiple images based on a text prompt.\n\nIf multiple images are requested and/or the generation will take a long time. The user can request to be notified\nonce the generated image(s) are ready." : "⚠️ Bu uygulama kullanımdan kaldırıldı. Sunduğu özellikler [Yardımcı uygulamasına](https://apps.nextcloud.com/apps/assistant) aktarıldı.\n\nBu uygulama, şirket içi kararlı dağıtım bütünleştirmesi gibi arayüze yazıdan görüntü oluşturma hizmeti sağlayıcıları için bir akıllı seçici ekler.\n\nAkıllı seçici, metin sorusuna göre bir ya da birkaç görsel oluşturmanızı sağlar.\n\nBirden fazla görsel isteniyorsa ve/veya oluşturulması uzun sürecekse. Kullanıcı, oluşturulan görseller hazır olduğunda\nbildirilmesini isteyebilir.",
    "Text2Image Helper admin options saved" : "Text2Image yardımcısı yönetici ayarları kaydedildi",
    "Failed to save Text2Image Helper admin options" : "Text2Image yardımcısı yönetici ayarları kaydedilemedi",
    "Image generation - Text2Image Helper" : "Görsel oluşturma - Text2Image yardımcısı",
    "Here you can define the settings for the Text2Image helper smart picker." : "Burada Text2Image yardımcısı akıllı seçicisini ayarlayabilirsiniz.",
    "Max generation idle time (days): " : "Oluşturmanın en fazla boşta bekleme süresi (gün):",
    "Maximum number of days an image can be idle (not viewed) before it is deleted" : "Bir görselin silinmeden önce boşta kalabileceği (görüntülenemeyeceği) en fazla gün sayısı",
    "Image generation failed" : "Görsel oluşturulamadı",
    "This generation has no visible images" : "Bu oluşturmada görülebilecek bir görsel yok",
    "Unexpected server response" : "Beklenmeyen bir sunucu yanıtı alındı",
    "Rate limit reached. Please try again later." : "Sınıra ulaşıldı. Lütfen bir süre sonra yeniden deneyin.",
    "Unknown server query error" : "Bilinmeyen sunucu sorgulama sorunu",
    "Image generation" : "Görsel oluşturma",
    "Edit visible images" : "Görülebilir görselleri düzenle",
    "Click to toggle generation visibility" : "Oluşturma görünürlüğünü değiştirmek için tıklayın",
    "Generated image" : "Oluşturulan görsel",
    "Estimated generation time left: " : "Oluşturma için öngörülen kalan süre:",
    "The generated image is shown once ready." : "Oluşturulan görsel hazır olduğunda görüntülenir.",
    "A hedgehog laid back on a couch" : "Bir kirpi kanepede gerindi",
    "Regenerate" : "Yeniden oluştur",
    "Preview" : "Ön izleme",
    "Image generation cancel error" : "Görsel oluşturma iptali sorunu",
    "Unknown image generation cancel error" : "Bilinmeyen görsel oluşturma iptali sorunu",
    "Unexpected response from server." : "Sunucudan beklenmeyen bir yanıt alındı.",
    "Image generation error" : "Görsel oluşturma sorunu",
    "Unknown image generation error" : "Bilinmeyen görsel oluşturma sorunu",
    "You will be notified when the image generation is ready." : "Oluşturulan görsel hazır olduğunda size bildirilecek.",
    "Notify when ready error" : "Hazır olduğunda bildirme sorunu",
    "Unknown notify when ready error" : "Bilinmeyen hazır olduğunda bildirme sorunu",
    "ERROR: No image generation provider available" : "HATA: Kullanılabilecek bir görsel oluşturma hizmeti sağlayıcısı yok",
    "AI Image Generation" : "Yapay Zeka ile görsel oluşturma",
    "Show/hide advanced options" : "Gelişmiş ayarları görüntüle/gizle",
    "Advanced options" : "Gelişmiş ayarlar",
    "Preview image generation by AI" : "Yapay Zeka ile görsel oluşturma ön izlemesi",
    "Notify when ready" : "Hazır olduğunda bilgilendir",
    "Submit image(s) generated by AI" : "Yapay Zeka ile oluşturulan görselleri gönder",
    "Send" : "Gönder",
    "Include the prompt in the result" : "Sonuca soru da katılsın",
    "Number of results" : "Sonuç sayısı",
    "Link copied to clipboard" : "Bağlantı panoya kopyalandı",
    "Failed to copy link to clipboard" : "Bağlantı panoya kopyalanamadı",
    "Copy link to clipboard" : "Bağlantıyı panoya kopyala"
},
"nplurals=2; plural=(n > 1);");
