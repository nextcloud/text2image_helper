		...
		<div v-if="pickerEnabled" class="text2image-picker-content">
			...
			<div class="footer">
				...
				<NcButton v-if="result !== null"
					type="primary"
					:aria-label="t('text2image_helper', 'Submit text generated by AI')"
			...
			if (answers.length > 0) {
				if (answers.length === 1) {
					this.result = this.includeQuery
						? t('text2image_helper', 'Prompt') + '\n' + this.query + '\n\n' + t('text2image_helper', 'Result') + '\n' + answers[0]
						: answers[0]
				} else {
					const multiAnswers = answers.map((a, i) => {
						return t('text2image_helper', 'Result {index}', { index: i + 1 }) + '\n' + a
					})
					this.result = this.includeQuery
						? t('text2image_helper', 'Prompt') + '\n' + this.query + '\n\n' + multiAnswers.join('\n\n')
						: multiAnswers.join('\n\n')
